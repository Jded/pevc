...
import { ModeActionTypes, SelectModeAction } from '../actions/mode.actions';
...
export class MaterialModeInteractionEffects {
  ...
  @Effect() modeChange$: Observable<Action> = this.actions$
    ...
    .map(([action , materialId]: [SelectModeAction, number]) => {
      ...
      return supported.indexOf(materialType) > -1 ? null : this.materialManagerService.defaultMaterials[supported[0]].id;
    })
    .filter( targetMaterial => targetMaterial !== null)
    .switchMap(targetMaterial =>
      Observable.of(new SelectActiveMaterialAction(targetMaterial))
    )
  ...
  @Effect() materialChange(): Observable<Action> {
    return this.actions$
      ...
      .filter( targetMode => targetMode !== null)
      .switchMap(targetMode =>
        Observable.of(new SelectModeAction(targetMode))
      )
  }

  @Effect() modelInputChange(): Observable<Action> {
    return this.actions$
  ...
  constructor(private actions$: Actions,
              ...
              private modelInputStore$: Store<ModelValueOverride>,
