...
export class HarvesterE33 implements DistortionMode {
  ...
  static api: ModeApi = Object.assign( new ModeApi(), {
    ...
    electricalDampingCoefficient: ModeApiValue.OUTPUT_SCALAR_STATIC
  });
  modeId: string;
  modeName: string;
  override: ModelValueDTO;
  api = HarvesterE33.api;
  ...
  distortModel(model: PlateService, time: number) {
    ...
    const ksqr33 = (model.material.e[2][2] * model.material.e[2][2]) / cepsilon;
    ...
    const capacitance = ( L * wS / hP ) *
      ( (model.material.epsilon[2][2] / Constants.EPSILON_ZERO)
        * Math.pow(10, Constants.EPSILON_EXP - Constants.EPSILON_ZERO_EXP))
    ...
    const electricalDampingCoeff = ksqr33 / (capacitance * resonantFrequency)

    model.modelValues.electricalDampingCoefficient = electricalDampingCoeff;
